import type { Star } from '../../types';
import { GRADE1_SENTENCES, GRADE1_WORDS } from '../generators/typing';

/**
 * Grade 1 maths and typing.
 *
 * Maths runs 30 questions per star against a clock, because fluency — not just
 * correctness — is the point at this stage. The time allowed is generous
 * (roughly 20 seconds a question) so it pushes without panicking, and running
 * out ends the star with the work already done rather than discarding it.
 * Multiple choice is off from early on: choosing between four numbers is a
 * different and easier skill than actually computing the answer.
 *
 * Typing starts far gentler than it used to. A six-year-old hunting for keys
 * needs single letters and two-letter clusters long before whole sentences.
 */

/**
 * 30 questions at roughly 30 seconds each.
 *
 * Deliberately generous for this age. The clock is there to build fluency, not
 * to create pressure — a six-year-old who feels chased stops thinking and
 * starts guessing, which practises exactly the wrong thing.
 */
const MATH_TIME = 900;

export const grade1Math: Star[] = [
  {
    id: 'g1_math_01', subject: 'math', grade: 1, minutes: 11, timeLimitSeconds: MATH_TIME,
    title: 'Counting Up to Ten',
    blurb: 'Thirty questions, and a clock. Do not rush — steady is faster than frantic.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-10', 'sub-within-10'], multipleChoice: true } },
  },
  {
    id: 'g1_math_02', subject: 'math', grade: 1, minutes: 11, timeLimitSeconds: MATH_TIME,
    title: 'Taking Away',
    blurb: 'Counting backwards is its own kind of brave.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['sub-within-10', 'sub-within-20'], multipleChoice: true } },
  },
  {
    id: 'g1_math_03', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'Past Ten',
    blurb: 'The numbers get bigger. You are ready for them.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20'], multipleChoice: true } },
  },
  {
    id: 'g1_math_04', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'The Missing Number',
    blurb: 'Something is hiding in each of these. Your job is to find it.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['missing-addend', 'add-within-20'], multipleChoice: true } },
  },
  {
    id: 'g1_math_05', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'Skip, Skip, Skip',
    blurb: 'Counting by twos, fives and tens. Patterns hiding inside numbers.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['skip-count', 'compare'] } },
  },
  {
    id: 'g1_math_06', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'Tens and Ones',
    blurb: 'Every two-digit number is made of tens and leftovers.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['place-value', 'compare'] } },
  },
  {
    id: 'g1_math_07', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Bigger Sums',
    blurb: 'Two-digit numbers. Add the tens first, then the ones.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit'] } },
  },
  {
    id: 'g1_math_08', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Bigger Take-Aways',
    blurb: 'Same idea, going down instead of up.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['sub-two-digit'] } },
  },
  {
    id: 'g1_math_09', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Story Numbers',
    blurb: 'The maths comes wrapped in a story. Listen for what is being asked.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['word-problem'] } },
  },
  {
    id: 'g1_math_10', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Coins and Cents',
    blurb: 'Money is just counting with a job to do.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['money'] } },
  },
  {
    id: 'g1_math_11', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Both Directions',
    blurb: 'Adding and taking away, all mixed up. Read the sign every time.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'missing-addend'] } },
  },
  {
    id: 'g1_math_12', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Number Detective',
    blurb: 'Missing numbers, comparisons and place value together.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['missing-addend', 'compare', 'place-value', 'skip-count'] } },
  },
  {
    id: 'g1_math_13', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Groups of Things',
    blurb: 'Adding the same number over and over. This is the beginning of times tables.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['skip-count', 'word-problem', 'add-two-digit'] } },
  },
  {
    id: 'g1_math_14', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Harder Stories',
    blurb: 'These take two steps. Do the first before you look for the second.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['word-problem', 'money'] } },
  },
  {
    id: 'g1_math_15', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Speed and Steadiness',
    blurb: 'Quick ones. Accuracy first — the clock is not the boss of you.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20', 'missing-addend'] } },
  },
  {
    id: 'g1_math_16', subject: 'math', grade: 1, minutes: 15, timeLimitSeconds: MATH_TIME,
    title: 'The Long Climb',
    blurb: 'Everything you know, thirty questions, one clock.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'skip-count', 'money', 'word-problem'] } },
  },
  {
    id: 'g1_math_17', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Reading the Problem',
    blurb: 'Every one of these is a story. Read it, picture it, then work it out.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['word-problem'] } },
  },
  {
    id: 'g1_math_18', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'How Many More',
    blurb: 'Stories where you have to work out the difference, not the total.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['word-problem', 'missing-addend'] } },
  },
  {
    id: 'g1_math_19', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Everything So Far',
    blurb: 'No hints about which kind is which. Read each one properly.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20', 'missing-addend', 'place-value', 'money', 'word-problem'] } },
  },
  {
    id: 'g1_math_20', subject: 'math', grade: 1, minutes: 15, timeLimitSeconds: MATH_TIME,
    title: 'The Counting Crown',
    blurb: 'Everything so far, all at once. And there is more ahead!',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'missing-addend', 'compare', 'place-value', 'skip-count', 'money', 'word-problem'] } },
  },
  {
    id: 'g1_math_21', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'Adding Practice Camp',
    blurb: 'Back to adding — but now you are faster than before.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-10', 'add-within-20'] } },
  },
  {
    id: 'g1_math_22', subject: 'math', grade: 1, minutes: 12, timeLimitSeconds: MATH_TIME,
    title: 'Take-Away Camp',
    blurb: 'Subtraction practice. Your brain knows the way now.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['sub-within-10', 'sub-within-20'] } },
  },
  {
    id: 'g1_math_23', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Mystery Numbers',
    blurb: 'More missing numbers to find. You are a number detective.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['missing-addend'] } },
  },
  {
    id: 'g1_math_24', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'The Comparing Game',
    blurb: 'Which is bigger? Which is smaller? Look carefully.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['compare', 'place-value'] } },
  },
  {
    id: 'g1_math_25', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Coin Counting',
    blurb: 'More money problems. Count those coins!',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['money'] } },
  },
  {
    id: 'g1_math_26', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: MATH_TIME,
    title: 'Skip Counting Trails',
    blurb: 'Twos, fives, and tens. Jump along the number line.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['skip-count'] } },
  },
  {
    id: 'g1_math_27', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Story Problem Theater',
    blurb: 'Every problem is a tiny story. Listen for the question.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['word-problem'] } },
  },
  {
    id: 'g1_math_28', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Two-Digit Power',
    blurb: 'Bigger numbers, stronger math muscles.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit'] } },
  },
  {
    id: 'g1_math_29', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Mixed-Up Monday',
    blurb: 'A little of everything, shuffled together.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20', 'compare', 'skip-count'] } },
  },
  {
    id: 'g1_math_30', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: MATH_TIME,
    title: 'Halfway Hero',
    blurb: 'You are halfway through the extra stars. Everything mixed!',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'missing-addend', 'money', 'word-problem'] } },
  },
  {
    id: 'g1_math_31', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: 780,
    title: 'Speedy Sums',
    blurb: 'Same adding, a little less time. You can do it.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-10', 'add-within-20'] } },
  },
  {
    id: 'g1_math_32', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: 780,
    title: 'Speedy Take-Aways',
    blurb: 'Quicker subtraction. Steady and sure wins.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['sub-within-10', 'sub-within-20'] } },
  },
  {
    id: 'g1_math_33', subject: 'math', grade: 1, minutes: 13, timeLimitSeconds: 780,
    title: 'Place Value Palace',
    blurb: 'Tens and ones, faster than ever.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['place-value', 'compare'] } },
  },
  {
    id: 'g1_math_34', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: 780,
    title: 'The Number Machine',
    blurb: 'Missing numbers and skip counting, mixed together.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['missing-addend', 'skip-count'] } },
  },
  {
    id: 'g1_math_35', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: 780,
    title: 'Market Day',
    blurb: 'Money and story problems — like shopping with your brain.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['money', 'word-problem'] } },
  },
  {
    id: 'g1_math_36', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: 720,
    title: 'Lightning Round One',
    blurb: 'Everything easy, extra quick. Trust your fingers.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20'] } },
  },
  {
    id: 'g1_math_37', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: 720,
    title: 'Lightning Round Two',
    blurb: 'Bigger numbers at lightning speed.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit'] } },
  },
  {
    id: 'g1_math_38', subject: 'math', grade: 1, minutes: 14, timeLimitSeconds: 720,
    title: 'The Great Mix-Up',
    blurb: 'No pattern to which kind comes next. Read each one!',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-within-20', 'sub-within-20', 'missing-addend', 'compare', 'place-value', 'skip-count'] } },
  },
  {
    id: 'g1_math_39', subject: 'math', grade: 1, minutes: 15, timeLimitSeconds: 720,
    title: 'Almost There',
    blurb: 'The second-to-last star. Everything you know, quick and confident.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'missing-addend', 'money', 'word-problem', 'skip-count'] } },
  },
  {
    id: 'g1_math_40', subject: 'math', grade: 1, minutes: 15, timeLimitSeconds: 700,
    title: 'The Grand Number Crown',
    blurb: 'The true final star. Every skill, best speed. You are a mathematician.',
    content: { kind: 'generated', count: 30, generator: { type: 'math', skills: ['add-two-digit', 'sub-two-digit', 'missing-addend', 'compare', 'place-value', 'skip-count', 'money', 'word-problem'] } },
  },
];

export const grade1Typing: Star[] = [
  {
    id: 'g1_type_01', subject: 'typing', grade: 1, minutes: 8,
    title: 'Just Four Keys',
    blurb: 'Rest your left fingers on A S D F. That is all we need today.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdf', rounds: 6 } },
  },
  {
    id: 'g1_type_02', subject: 'typing', grade: 1, minutes: 8,
    title: 'Four More Keys',
    blurb: 'Now the right hand. J K L, and nothing else.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'jkl', rounds: 6 } },
  },
  {
    id: 'g1_type_03', subject: 'typing', grade: 1, minutes: 8,
    title: 'Both Hands',
    blurb: 'Left and right together. Slow is perfectly fine.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdfjkl', rounds: 8 } },
  },
  {
    id: 'g1_type_04', subject: 'typing', grade: 1, minutes: 9,
    title: 'Two Little Words',
    blurb: 'Real words now. Only the shortest ones.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['a', 'as', 'ask', 'all', 'fall', 'lad', 'salad', 'flask'], rounds: 8 } },
  },
  {
    id: 'g1_type_05', subject: 'typing', grade: 1, minutes: 9,
    title: 'Adding E and I',
    blurb: 'Two new letters. Your fingers reach up just a little.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdfjklei', rounds: 8 } },
  },
  {
    id: 'g1_type_06', subject: 'typing', grade: 1, minutes: 9,
    title: 'Words You Know',
    blurb: 'Words you can already read. Now your fingers learn them.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['the', 'and', 'see', 'like', 'is', 'it', 'in', 'at', 'me', 'we'], rounds: 8 } },
  },
  {
    id: 'g1_type_07', subject: 'typing', grade: 1, minutes: 9,
    title: 'More Letters',
    blurb: 'A few more keys join in.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdfjkleirumn', rounds: 8 } },
  },
  {
    id: 'g1_type_08', subject: 'typing', grade: 1, minutes: 10,
    title: 'Little Words',
    blurb: 'Short words, typed one letter at a time.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: GRADE1_WORDS.slice(0, 16), rounds: 8 } },
  },
  {
    id: 'g1_type_09', subject: 'typing', grade: 1, minutes: 10,
    title: 'Colour Words',
    blurb: 'Red, blue, green. Words you use every day.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['red', 'blue', 'green', 'one', 'two', 'three', 'sun', 'moon', 'star', 'cat', 'dog'], rounds: 8 } },
  },
  {
    id: 'g1_type_10', subject: 'typing', grade: 1, minutes: 10,
    title: 'The Whole Alphabet',
    blurb: 'Every letter now. Take your time and look for each one.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'abcdefghijklmnopqrstuvwxyz', rounds: 8 } },
  },
  {
    id: 'g1_type_11', subject: 'typing', grade: 1, minutes: 11,
    title: 'All the Words',
    blurb: 'Everything from every word star so far.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: GRADE1_WORDS, rounds: 10 } },
  },
  {
    id: 'g1_type_12', subject: 'typing', grade: 1, minutes: 11,
    title: 'Your First Sentence',
    blurb: 'A capital letter needs Shift. Hold it with the other hand.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: GRADE1_SENTENCES.slice(0, 4) } },
  },
  {
    id: 'g1_type_13', subject: 'typing', grade: 1, minutes: 12,
    title: 'More Sentences',
    blurb: 'Remember the full stop at the end.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: GRADE1_SENTENCES.slice(4, 8) } },
  },
  {
    id: 'g1_type_14', subject: 'typing', grade: 1, minutes: 12,
    title: 'The Swift Hand',
    blurb: 'Every sentence you have learned. Now the practice continues.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: GRADE1_SENTENCES } },
  },
  {
    id: 'g1_type_15', subject: 'typing', grade: 1, minutes: 10,
    title: 'Top Row Letters',
    blurb: 'Q W E R T Y. The letters above your home row.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'qwerty', rounds: 8 } },
  },
  {
    id: 'g1_type_16', subject: 'typing', grade: 1, minutes: 10,
    title: 'More Top Row',
    blurb: 'U I O P join the practice.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'uiop', rounds: 8 } },
  },
  {
    id: 'g1_type_17', subject: 'typing', grade: 1, minutes: 10,
    title: 'Home and Top Together',
    blurb: 'Both rows now. Your fingers are learning the map.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdfjklqwertyuiop', rounds: 8 } },
  },
  {
    id: 'g1_type_18', subject: 'typing', grade: 1, minutes: 10,
    title: 'Bottom Row Letters',
    blurb: 'Z X C V B N M. The letters below your fingers.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'zxcvbnm', rounds: 8 } },
  },
  {
    id: 'g1_type_19', subject: 'typing', grade: 1, minutes: 10,
    title: 'Animal Words',
    blurb: 'Type the names of animals you know.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['cat', 'dog', 'pig', 'cow', 'hen', 'fox', 'owl', 'bee', 'ant', 'bat'], rounds: 8 } },
  },
  {
    id: 'g1_type_20', subject: 'typing', grade: 1, minutes: 10,
    title: 'Family Words',
    blurb: 'Words for the people you love.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['mom', 'dad', 'sister', 'brother', 'baby', 'family', 'grandma', 'grandpa', 'aunt', 'uncle'], rounds: 8 } },
  },
  {
    id: 'g1_type_21', subject: 'typing', grade: 1, minutes: 10,
    title: 'Food Words',
    blurb: 'Yummy words to type.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['apple', 'bread', 'milk', 'egg', 'cake', 'rice', 'soup', 'corn', 'pear', 'plum'], rounds: 8 } },
  },
  {
    id: 'g1_type_22', subject: 'typing', grade: 1, minutes: 10,
    title: 'Weather Words',
    blurb: 'Sun, rain, snow — type them all.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['sun', 'rain', 'snow', 'wind', 'cloud', 'storm', 'hot', 'cold', 'warm', 'wet'], rounds: 8 } },
  },
  {
    id: 'g1_type_23', subject: 'typing', grade: 1, minutes: 11,
    title: 'Action Words',
    blurb: 'Words that move — run, jump, hop!',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['run', 'jump', 'hop', 'skip', 'walk', 'swim', 'clap', 'sing', 'dance', 'spin'], rounds: 8 } },
  },
  {
    id: 'g1_type_24', subject: 'typing', grade: 1, minutes: 11,
    title: 'Happy Sentences',
    blurb: 'Short cheerful sentences to type.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'I love my family.',
      'The dog is happy.',
      'We play outside.',
      'The cake is sweet.',
    ] } },
  },
  {
    id: 'g1_type_25', subject: 'typing', grade: 1, minutes: 11,
    title: 'Animal Sentences',
    blurb: 'Type about the animals.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'The cat sat on the mat.',
      'A bird can fly high.',
      'The fish swims fast.',
      'My dog likes to run.',
    ] } },
  },
  {
    id: 'g1_type_26', subject: 'typing', grade: 1, minutes: 11,
    title: 'Question Sentences',
    blurb: 'Sentences that ask something end with a question mark.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'Can you see the moon?',
      'Where is my book?',
      'What is your name?',
      'Do you like to play?',
    ] } },
  },
  {
    id: 'g1_type_27', subject: 'typing', grade: 1, minutes: 11,
    title: 'Nature Sentences',
    blurb: 'Type about the world outside.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'The tree is very tall.',
      'Flowers grow in spring.',
      'The river runs to the sea.',
      'Stars shine at night.',
    ] } },
  },
  {
    id: 'g1_type_28', subject: 'typing', grade: 1, minutes: 12,
    title: 'Longer Words',
    blurb: 'Bigger words for stronger fingers.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['morning', 'evening', 'together', 'wonderful', 'birthday', 'rainbow', 'butterfly', 'sunshine', 'playground', 'friend'], rounds: 8 } },
  },
  {
    id: 'g1_type_29', subject: 'typing', grade: 1, minutes: 12,
    title: 'Story Sentences',
    blurb: 'Sentences that tell a tiny story.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'Once there was a little star.',
      'The star wanted a friend.',
      'A comet came to visit.',
      'Now they shine together.',
    ] } },
  },
  {
    id: 'g1_type_30', subject: 'typing', grade: 1, minutes: 12,
    title: 'Number Row',
    blurb: 'The numbers live above the letters. Reach up and find them.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: '1234567890', rounds: 8 } },
  },
  {
    id: 'g1_type_31', subject: 'typing', grade: 1, minutes: 12,
    title: 'Letters and Numbers',
    blurb: 'Mixing letters with numbers, like a secret code.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'keys', keys: 'asdfjkl1234567890', rounds: 8 } },
  },
  {
    id: 'g1_type_32', subject: 'typing', grade: 1, minutes: 12,
    title: 'School Words',
    blurb: 'Words from your classroom.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'words', words: ['school', 'teacher', 'pencil', 'paper', 'desk', 'chair', 'class', 'lunch', 'recess', 'learn'], rounds: 8 } },
  },
  {
    id: 'g1_type_33', subject: 'typing', grade: 1, minutes: 12,
    title: 'Long Sentences',
    blurb: 'The longest sentences yet. Take them one word at a time.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'The little bird flew over the tall green trees.',
      'My best friend and I like to play outside.',
      'We saw three stars in the sky last night.',
      'The happy dog ran fast around the big yard.',
    ] } },
  },
  {
    id: 'g1_type_34', subject: 'typing', grade: 1, minutes: 12,
    title: 'The Golden Keys',
    blurb: 'The final typing star. Everything you have learned, all at once.',
    content: { kind: 'generated', count: 0, generator: { type: 'typing', mode: 'sentence', sentences: [
      'I can type all by myself now.',
      'My fingers know where to go.',
      'Practice makes me better every day.',
      'The little star typed her way home.',
    ] } },
  },
];
